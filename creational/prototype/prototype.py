...
class Robot(CopyableInterface):
    ...
    def __copy__(self):
        ...
        new_object = self.__class__(self.height, self.weight, known_phrases)
...
if __name__ == "__main__":
    ...
    assert id(robot) != id(
        shallow_copy_robot
    ), "Copied object must be in another memory location"
    assert id(robot) != id(
        deepcopy_robot
    ), "Copied object must be in another memory location"

    assert id(robot.known_phrases) == id(
        shallow_copy_robot.known_phrases
    ), "Shallow copy doesn't copy nested lists"
    assert id(robot.known_phrases) != id(
        deepcopy_robot.known_phrases
    ), "Deep copy must copy nested lists"
